...
const GRAD = "linear-gradient(135deg, #CB3273 0%, #E63980 100%)";
const PURPLE = "#CB3273";
...
function FeedbackModal({ onClose }) {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.55)" }} onClick={e => { if (e.target === e.currentTarget)
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        {/* Header */}
        <div className="px-5 py-4 flex items-center justify-between" style={{ background: "linear-gradient(135deg,#A4285E,#CB3273)" }}>
...
function FeedbackButton() {
    ...
    return (<>
      <button onClick={() => setOpen(true)} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[12px] font-bold transition-all hover:opacity-90" style={{ background: "linear-gradient(135deg,#A4285E,#CB3273)", color: "#fff" }}>
...
function BookingLinkButton() {
    ...
    return (<div className="rounded-lg overflow-hidden" style={{ background: "linear-gradient(135deg, #A4285E, #CB3273)" }}>
        ...
        {[
            ...
            <div className="p-3 rounded-xl border-2 cursor-pointer transition-all text-left hover:shadow-md" style={{ borderColor: plan.highlight ? "#CB3273" : "#E5E7EB", background: plan.highlight ? "#FBE9F1" : "#fff" }}>
              ...
              <div className="mt-0.5">
                <span className="text-[14px] font-black" style={{ color: plan.highlight ? "#CB3273" : "#111827" }}>{plan.price}</span>
                <span className="text-[9px] text-gray-400">/mo</span>
              </div>
              <div className="text-[9px] text-gray-500 mt-0.5 leading-tight">{plan.desc}</div>
              {plan.highlight && <div className="mt-1.5 text-[9px] font-bold px-1.5 py-0.5 rounded-full text-center" style={{ background: "#CB3273", color: "#fff" }}>Most Popular</div>}
            </div>
          </Link>))}
      </div>
      <Link href="/billing">
        <button className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-white text-sm" style={{ background: "linear-gradient(135deg,#A4285E,#CB3273)" }}>
