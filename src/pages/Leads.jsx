          ...
          <div className="rounded-xl border border-gray-200 w-full max-w-2xl bg-white shadow-sm" style={{ maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-5">
              ...
              <div className="flex gap-2 pt-1 border-t border-gray-100">
                ...
                <button type="submit" disabled={createLead.isPending} className="flex-1 px-3 py-2 rounded-lg text-xs text-white font-semibold" style={{ background: "#A4285E" }}>
      ...
      <div className="fixed right-0 top-0 h-full z-50 flex flex-col" style={{ width: "min(490px, 100vw)", background: D.panel, borderLeft: `1px solid ${D.cardBorder}`, boxShadow: "-8px 0 48px rgba(0,0,0,0.6)" }}>
        ...
        <div className="flex-shrink-0 relative overflow-hidden px-5 pt-5 pb-4" style={{ background: D.header, borderBottom: `1px solid ${D.divider}` }}>
          ...
          <div className="relative flex items-start gap-3.5">
            ...
            background: `linear-gradient(135deg, #4A1030 0%, #2A0A18 100%)`,
            boxShadow: `0 0 0 1px ${D.cardBorder}, 0 0 20px rgba(229,139,181,0.2)`,
            color: D.accent,
        ...
        <div className="flex-1 overflow-y-auto" style={{ background: D.panel, scrollbarWidth: "thin", scrollbarColor: `${D.divider} transparent` }}>
          {isLoading ? (<div className="p-5 space-y-3">
              ...
              <DarkCard title="AI Intelligence" d={D}>
                ...
                ? (lead.intentKeywords).map((k) => (<span key={k} className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: "#1E1645", color: "#F3C9DB", border: "1px solid #3D2D6E" }}>{k}</span>))
                ...
                ? (lead.behaviorKeywords).map((k) => (<span key={k} className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: "#2A0A18", color: "#FBE9F1", border: "1px solid #5A1232" }}>{k}</span>))
                : <span className="text-[11px]" style={{ color: D.textDim }}>None yet</span>}
                    </div>
                  </div>
                  <div>
...
function ExportModal({ rowCount, defaultName, onExport, onClose, }) {
    ...
    return (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100" style={{ background: "#A4285E" }}>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={filename} onChange={(e) => setFilename(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#A4285E]/20 focus:border-[#A4285E]"/>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {["xlsx", "csv"].map((f) => (<button key={f} onClick={() => setFormat(f)} className={cn("flex items-center gap-2.5 px-4 py-3 rounded-xl border text-[13px] font-medium transition-all", format === f
                ? "border-[#A4285E] bg-[#A4285E]/5 text-[#A4285E]"
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
          <button onClick={() => onExport(format, (filename.trim() || "leads").replace(/\.(csv|xlsx?)$/i, ""))} disabled={!filename.trim()} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[13px] font-bold text-white transition-all disabled:opacity-50" style={{ background: "#A4285E" }}>
