...
function Title({ raw }) {
    ...
    return (<h2 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: "clamp(22px,4vw,30px)", lineHeight: 1.22, color: "#fff", marginBottom: 10 }}>
      {parts.map((p, i) => i % 2 === 1
            ? <span key={i} style={{ background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{p}</span>
            : p)}
    </h2>);
}
/* ── Main component ──────────────────────────────────────────────────── */
export default function GrowthQuestForm() {
    ...
    return (<>
      ...
      <style>{`
        ...
        .gq-cta{width:100%;padding:18px;border-radius:16px;border:none;background:linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC);color:#fff;font-family:'Sora',sans-serif;font-size:15px;font-weight:700;cursor:pointer;transition:all .25s;box-shadow:0 8px 36px rgba(255,107,53,.32);position:relative;overflow:hidden}
      ...
      <div className="gq-body">
        ...
        <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
          {[
            ...
            { size: 400, color: "#FF3CAC", top: "35%", left: "45%", delay: "-7s" },
            { size: 300, color: "#E8304A", top: "60%", right: "10%", delay: "-2s" },
        ...
        <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", padding: "0 16px 100px" }}>
          ...
          <nav style={{ maxWidth: 780, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px 0 18px" }}>
            <a href="https://dreamsdesign.in" target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
              <div style={{ width: 40, height: 40, borderRadius: 11, background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 16, color: "#fff" }}>DD</div>
              <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 17, color: "#fff" }}>
                Dreams<span style={{ background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>design</span>
              </span>
            </a>
            <div style={{ display: "flex", alignItems: "center", gap: 9, background: "rgba(255,107,53,.08)", border: "1px solid rgba(255,107,53,.3)", borderRadius: 50, padding: "9px 18px" }}>
              ...
              <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 17, background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{coins}</span>
          ...
          <div style={{ maxWidth: 780, margin: "0 auto 28px" }}>
            ...
            <div style={{ height: 4, background: "rgba(255,255,255,.07)", borderRadius: 2, position: "relative" }}>
              <div style={{ width: `${pct}%`, height: "100%", borderRadius: 2, background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", transition: "width .6s cubic-bezier(.4,0,.2,1)", position: "relative" }}>
                <div style={{ position: "absolute", right: -7, top: "50%", transform: "translateY(-50%)", width: 14, height: 14, borderRadius: "50%", background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", boxShadow: "0 0 16px rgba(255,107,53,.9),0 0 32px rgba(255,107,53,.4)" }}/>
              </div>
            </div>
          </div>
...
function QuestionCard({ step, answers, setAnswers, onAdvance }) {
    ...
    const card = (<div className="gq-card" style={{ maxWidth: 780, margin: "0 auto", background: "#14141E", border: "1px solid rgba(255,107,53,.2)", borderRadius: 26, padding: "clamp(24px,5vw,44px) clamp(20px,5vw,40px)", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)" }}/>
    ...
    return (<>
      {answers.name && (<div style={{ background: "rgba(255,107,53,.06)", border: "1px solid rgba(255,107,53,.22)", borderRadius: 14, padding: "14px 18px", marginBottom: 16, animation: "fadeScale .35s ease" }}>
          <span style={{ fontFamily: "'Sora',sans-serif", fontSize: 15, fontWeight: 600, color: "rgba(255,255,255,.8)" }}>
            The Hero's name is{" "}
            <span style={{ background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontSize: 19 }}>{answers.name}</span>
...
function SingleStep({ stepId, opts, answers, update, onAdvance }) {
    ...
    return (<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 10, marginBottom: 24 }} className="gq-opts-grid">
      {opts.map(o => (<button key={o.l} className={`gq-opt${sel === o.l ? " sel" : ""}`} style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 14, padding: "15px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer", textAlign: "left", color: "rgba(255,255,255,.6)", fontSize: 13.5, fontWeight: 500, fontFamily: "'DM Sans',sans-serif", transition: "all .22s", position: "relative", overflow: "hidden" }} onClick={() => { update({ [stepId]: o.l }); setTimeout(onAdvance, 250); }}>
          ...
          <span style={{ width: 19, height: 19, borderRadius: "50%", flexShrink: 0, border: `1.5px solid ${sel === o.l ? "transparent" : "rgba(255,255,255,.18)"}`, display: "flex", alignItems: "center", justifyContent: "center", background: sel === o.l ? "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)" : "transparent", transition: "all .2s" }}>
...
function FinalScreen({ answers, code, submitted, submitting, onSubmit, onWhatsApp }) {
    ...
    return (<div className="gq-final" style={{ maxWidth: 780, margin: "0 auto", background: "#14141E", border: "1px solid rgba(255,107,53,.2)", borderRadius: 26, padding: "clamp(32px,6vw,52px) clamp(22px,5vw,44px)", textAlign: "center", position: "relative", overflow: "hidden" }}>
      ...
      <h1 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: "clamp(24px,5vw,34px)", color: "#fff", lineHeight: 1.2, marginBottom: 14 }}>
        You've earned{" "}
        <span style={{ background: "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>1000 Gold Coins,</span>
        <br />{hero}!
      </h1>
      ...
      <div style={{ background: "rgba(255,107,53,.05)", border: "1.5px dashed rgba(255,107,53,.38)", borderRadius: 22, padding: "28px 26px", marginBottom: 28, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: "clamp(24px,6vw,38px)", background: "linear-gradient(135deg,#FFD200,#FF8C00,#FF6B35,#FF3CAC)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: 6 }}>{code}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <button style={{ width: "100%", padding: 19, borderRadius: 16, border: "none", background: submitted ? "rgba(255,107,53,.2)" : "linear-gradient(135deg,#FF6B35,#E8304A,#FF3CAC)", color: submitted ? "#FF6B35" : "#fff", fontFamily: "'Sora',sans-serif", fontSize: 15, fontWeight: 700, cursor: submitted ? "default" : "pointer", boxShadow: submitted ? "none" : "0 8px 36px rgba(255,107,53,.32)", transition: "all .2s", opacity: submitting ? 0.6 : 1 }} onClick={() => !submitted && !submitting && onSubmit()} disabled={submitting}>
